...
[![figure-3.4](https://mcxiaoke.gitbooks.io/mqtt-cn/content/mqtt/images/figure0304.png)](https://mcxiaoke.gitbooks.io/mqtt-cn/content/mqtt/0301-CONNECT.html)
...
[图例 3.10 – PUBLISH报文固定报头](https://mcxiaoke.gitbooks.io/mqtt-cn/content/mqtt/0303-PUBLISH.html#_图例_3.10_–)描述了固定报头的格式
...
可变报头包含等待确认的SUBSCRIBE报文的报文标识符。[图例 3.25 – SUBACK报文可变报头](https://mcxiaoke.gitbooks.io/mqtt-cn/content/mqtt/0309-SUBACK.html#_图例_3.25_–) 描述了可变报头的格式。
...
> **非规范评注**
>
> MQTT 3.1使用的传输层协议是 [[RFC793](https://mcxiaoke.gitbooks.io/mqtt-cn/content/mqtt/04-OperationalBehavior.html#RFC793)] 定义的TCP/IP协议。下面的协议也支持：

- TLS协议 [[RFC5246\]](https://mcxiaoke.gitbooks.io/mqtt-cn/content/mqtt/04-OperationalBehavior.html#RFC5246)
- WebSocket协议 [[RFC6455\]](https://mcxiaoke.gitbooks.io/mqtt-cn/content/mqtt/04-OperationalBehavior.html#RFC6455)
...
>
> [图例 4.3 – QoS 2协议流程图，非规范示例](https://mcxiaoke.gitbooks.io/mqtt-cn/content/mqtt/04-OperationalBehavior.html#_Figure_4.3_–) 展示了接收者对QoS 2等级消息的两种处理方法。他们的区别是消息什么时候可以开始分发。实现者可以决定使用哪种方法。只要实现者只选择了一种方法，就不会影响QoS流程的可靠性。
...
- 主题名和主题过滤器**不能**包含空字符 (Unicode U+0000) [[Unicode](https://mcxiaoke.gitbooks.io/mqtt-cn/content/mqtt/04-OperationalBehavior.html#Unicode)] [MQTT-4.7.3-2]。
...
- [第一章 - 介绍](https://mcxiaoke.gitbooks.io/mqtt-cn/content/mqtt/01-Introduction.html)
- [第二章 – MQTT控制报文格式](https://mcxiaoke.gitbooks.io/mqtt-cn/content/mqtt/02-ControlPacketFormat.html)
- [第三章 – MQTT控制报文](https://mcxiaoke.gitbooks.io/mqtt-cn/content/mqtt/03-ControlPackets.html)
- [第四章 – 操作行为](https://mcxiaoke.gitbooks.io/mqtt-cn/content/mqtt/04-OperationalBehavior.html)
- [第五章 – 安全](https://mcxiaoke.gitbooks.io/mqtt-cn/content/mqtt/05-Security.html)
- [第六章 – 使用WebSocket](https://mcxiaoke.gitbooks.io/mqtt-cn/content/mqtt/06-WebSocket.html)
- [第七章 – 一致性目标](https://mcxiaoke.gitbooks.io/mqtt-cn/content/mqtt/07-Conformance.html)
- [附录B - 强制性规范声明](https://mcxiaoke.gitbooks.io/mqtt-cn/content/mqtt/08-AppendixB.html)
...
作为传输层协议，MQTT仅关注消息传输，提供合适的安全功能是实现者的责任。使用TLS [[RFC5246\]](https://mcxiaoke.gitbooks.io/mqtt-cn/content/mqtt/05-Security.html#RFC5246) 是比较普遍的选择。除了技术上的安全问题外，还有地理因素（例如美国欧盟安全港原则 [[USEUSAFEHARB\]](https://mcxiaoke.gitbooks.io/mqtt-cn/content/mqtt/05-Security.html#USEUSAFEHARB)），行业标准（例如第三方支付行业数据安全标准 [[PCIDSS\]](https://mcxiaoke.gitbooks.io/mqtt-cn/content/mqtt/05-Security.html#PCIDSS)），监管方面的考虑（例如萨班斯-奥克斯利法案 [[SARBANES\]](https://mcxiaoke.gitbooks.io/mqtt-cn/content/mqtt/05-Security.html#SARBANES)）等问题。

## 5.2 MQTT解决方案：安全和认证

MQTT solutions: security and certification

协议实现可能想要符合特定的工业安全标准，如NIST网络安全框架 [[NISTCSF\]](https://mcxiaoke.gitbooks.io/mqtt-cn/content/mqtt/05-Security.html#NISTCSF) ，第三方支付行业数据安全标准 [[PCIDSS\]](https://mcxiaoke.gitbooks.io/mqtt-cn/content/mqtt/05-Security.html#PCIDSS) ，美国联邦信息处理标准 [[FIPS1402\]](https://mcxiaoke.gitbooks.io/mqtt-cn/content/mqtt/05-Security.html#FIPS1402) 和 NSA 加密组合B [[NSAB](https://mcxiaoke.gitbooks.io/mqtt-cn/content/mqtt/05-Security.html#NSAB)] 。

在MQTT的补充出版物 （MQTT and the NIST Framework for Improving Critical Infrastructure Cybersecurity [[MQTT NIST\]](https://mcxiaoke.gitbooks.io/mqtt-cn/content/mqtt/05-Security.html#NISTCSF)） 中可以找到在NIST网络安全框架 [[NISTCSF\]](https://mcxiaoke.gitbooks.io/mqtt-cn/content/mqtt/05-Security.html#NISTCSF) 中使用MQTT的指导。使用行业证明、独立审计和认证技术有助于满足合规要求。

## 5.3 轻量级的加密与受限设备

Lightweight cryptography and constrained devices

广泛采用高级加密标准 [[AES\]](https://mcxiaoke.gitbooks.io/mqtt-cn/content/mqtt/05-Security.html#AES) 数据加密标准 [[DES\]](https://mcxiaoke.gitbooks.io/mqtt-cn/content/mqtt/05-Security.html#DES) 。

推荐使用为受限的低端设备特别优化过的轻量级加密国际标准 ISO 29192 [[ISO29192\]](https://mcxiaoke.gitbooks.io/mqtt-cn/content/mqtt/05-Security.html#ISO29192) 。
...
CONNECT报文包含用户名和密码字段。实现可以决定如何使用这些字段的内容。实现者可以提供自己的身份验证机制，或者使用外部的认证系统如LDAP [[RFC4511\]](https://mcxiaoke.gitbooks.io/mqtt-cn/content/mqtt/05-Security.html#RFC4511) 或OAuth [[RFC6749\]](https://mcxiaoke.gitbooks.io/mqtt-cn/content/mqtt/05-Security.html#RFC6749) ，还可以利用操作系统的认证机制。

实现可以明文传递认证数据，混淆那些数据，或者不要求任何认证数据，但应该意识到这会增加中间人攻击和重放攻击的风险。5.4.5节介绍了确保数据私密的方法。

在客户端和服务端之间使用虚拟专用网（VPN）可以确保数据只被授权的客户端收到。

使用TLS [[RFC5246\]](https://mcxiaoke.gitbooks.io/mqtt-cn/content/mqtt/05-Security.html#RFC5246) 时，服务端可以使用客户端发送的SSL证书验证客户端的身份。
...
但是使用TLS [[RFC5246\]](https://mcxiaoke.gitbooks.io/mqtt-cn/content/mqtt/05-Security.html#RFC5246) 时，客户端可以使用服务端发送的SSL证书验证服务端的身份。从单IP多域名提供MQTT服务的实现应该考虑RFC6066 [[RFC6066\]](https://mcxiaoke.gitbooks.io/mqtt-cn/content/mqtt/05-Security.html#RFC6066) 第3节定义的TLS的SNI扩展。SNI允许客户端告诉服务端它要连接的服务端主机名。
...
TLS [[RFC5246\]](https://mcxiaoke.gitbooks.io/mqtt-cn/content/mqtt/05-Security.html#RFC5246) 提供了对网络传输的数据做完整性校验的哈希算法。

在客户端和服务端之间使用虚拟专用网（VPN）连接可以在VPN覆盖的网络段提供数据完整性检查。

### 5.4.5 控制报文和应用消息的保密性 Privacy of Application Messages and Control Packets

TLS [[RFC5246\]](https://mcxiaoke.gitbooks.io/mqtt-cn/content/mqtt/05-Security.html#RFC5246) 可以对网络传输的数据加密。如果有效的TLS密码组合包含的加密算法为NULL，那么它不会加密数据。要确保客户端和服务端的保密，应避免使用这些密码组合。
...
使用TLS [[RFC5246\]](https://mcxiaoke.gitbooks.io/mqtt-cn/content/mqtt/05-Security.html#RFC5246) 的客户端和服务端实现应该能够确保，初始化TLS [[RFC5246\]](https://mcxiaoke.gitbooks.io/mqtt-cn/content/mqtt/05-Security.html#RFC5246) 连接时提供的SSL证书是与主机名（客户端要连接的或服务端将被连接的）关联的。

使用TLS [[RFC5246\]](https://mcxiaoke.gitbooks.io/mqtt-cn/content/mqtt/05-Security.html#RFC5246) 的客户端和服务端实现，可以选择提供检查证书吊销列表 (CRLs [[RFC5280\]](https://mcxiaoke.gitbooks.io/mqtt-cn/content/mqtt/05-Security.html#RFC5280)) 和在线证书状态协议 (OSCP) [[RFC6960\]](https://mcxiaoke.gitbooks.io/mqtt-cn/content/mqtt/05-Security.html#RFC6960) 的功能，拒绝使用被吊销的证书。

物理部署可以将防篡改硬件与应用消息的特殊数据传输结合。例如，一个仪表可能会内置一个GPS以确保没有在未授权的地区使用。IEEE安全设备认证 [[IEEE 802.1AR\]](https://mcxiaoke.gitbooks.io/mqtt-cn/content/mqtt/05-Security.html#IEEE8021AR) 就是用于实现这个机制的一个标准，它使用加密绑定标识符验证设备身份。
...
如果客户端或服务端的SSL证书丢失，或者我们考虑证书被盗用或者被吊销(利用 CRLs [[RFC5280\]](https://mcxiaoke.gitbooks.io/mqtt-cn/content/mqtt/05-Security.html#RFC5280) 和 OSCP [[RFC6960\]](https://mcxiaoke.gitbooks.io/mqtt-cn/content/mqtt/05-Security.html#RFC6960))的情况。

客户端或服务端验证凭证时，如果发现用户名和密码丢失或被盗用，应该吊销或者重新发放。

在使用长连接时：

- 客户端和服务端使用TLS [[RFC5246\]](https://mcxiaoke.gitbooks.io/mqtt-cn/content/mqtt/05-Security.html#RFC5246) 时应该允许重新协商会话以确认新的加密参数（替换会话密钥，更换密码组合，更换认证凭证）。
- 服务端可以断开客户端连接，并要求他们使用新的凭证重新验证身份。

受限网络上的受限设备和客户端可以使用TLS会话恢复 [[RFC5077\]](https://mcxiaoke.gitbooks.io/mqtt-cn/content/mqtt/05-Security.html#RFC5077) 降低TLS会话重连 [[RFC5246\]](https://mcxiaoke.gitbooks.io/mqtt-cn/content/mqtt/05-Security.html#RFC5246) 的成本。

连接到服务端的客户端与其它连接到服务端的客户端 之间有一个信任传递关系，它们都有权在同一个主题上发布消息。

### 5.4.10 使用SOCKS代理 Use of SOCKS

客户端实现应该意识到某些环境要求使用SOCKSv5 [[RFC1928\]](https://mcxiaoke.gitbooks.io/mqtt-cn/content/mqtt/05-Security.html#RFC1928) 代理创建出站的网络连接。某些MQTT实现可以利用安全隧道（如SSH）通过SOCKS代理。一个实现决定支持SOCKS时，它们应该同时支持匿名的和用户名密码验证的SOCKS代理。对于后一种情况，实现应该意识到SOCKS可能使用明文认证，因此应该避免使用相同的凭证连接MQTT服务器。
...
使用安全传输配置时，MQTT协议运行在使用TLS [[RFC5246\]](https://mcxiaoke.gitbooks.io/mqtt-cn/content/mqtt/05-Security.html#RFC5246) 的物理或虚拟网络上，它提供了身份认证，完整性和保密性。

使用内置的用户名和密码字段，TLS [[RFC5246\]](https://mcxiaoke.gitbooks.io/mqtt-cn/content/mqtt/05-Security.html#RFC5246) 客户端身份认证可被用于（或者代替）MQTT客户端认证。

#### 工业标准的安全配置

可以预料的是，MQTT被设计为支持很多工业标准的应用配置，每一种定义一个威胁模型和用于定位威胁的特殊安全机制。特殊的安全机制推荐从下面的方案中选择：

[[NISTCSF\]](https://mcxiaoke.gitbooks.io/mqtt-cn/content/mqtt/05-Security.html#NISTCSF) NIST网络安全框架 [[NIST7628\]](https://mcxiaoke.gitbooks.io/mqtt-cn/content/mqtt/05-Security.html#NIST7628) NISTIR 7628智能电网网络安全指南 [[FIPS1402\]](https://mcxiaoke.gitbooks.io/mqtt-cn/content/mqtt/05-Security.html#FIPS1402) (FIPS PUB 140-2) 加密模块的安全要求 [[PCIDSS\]](https://mcxiaoke.gitbooks.io/mqtt-cn/content/mqtt/05-Security.html#PCIDSS) PCI-DSS第三方支付行业数据安全标准 [[NSAB\]](https://mcxiaoke.gitbooks.io/mqtt-cn/content/mqtt/05-Security.html#NSAB) NSA加密组合B

# 第六章 使用WebSocket作为网络层

如果MQTT在WebSocket [[RFC6455\]](https://mcxiaoke.gitbooks.io/mqtt-cn/content/mqtt/06-WebSocket.html#RFC6455) 连接上传输，**必须**满足下面的条件：
...
| **声明序号**    | **规范声明**                                                 |
| --------------- | ------------------------------------------------------------ |
| [MQTT-1.5.3-1]  | UTF-8编码字符串中的字符数据**必须**是按照Unicode规范 [[Unicode](https://mcxiaoke.gitbooks.io/mqtt-cn/content/mqtt/08-AppendixB.html#Unicode)] 定义的和在RFC3629 [[RFC3629](https://mcxiaoke.gitbooks.io/mqtt-cn/content/mqtt/08-AppendixB.html#RFC3629)] 中重申的有效的UTF-8格式。特别需要指出的是，这些数据**不能**包含字符码在U+D800和U+DFFF之间的数据。如果服务端或客户端收到了一个包含无效UTF-8字符的控制报文，它**必须**关闭网络连接。 |
...
| [MQTT-4.7.3-2]  | 主题名和主题过滤器**不能**包含空字符 (Unicode U+0000) [[Unicode](https://mcxiaoke.gitbooks.io/mqtt-cn/content/mqtt/08-AppendixB.html#Unicode)] 。 |
